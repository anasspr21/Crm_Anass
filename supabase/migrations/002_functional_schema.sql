-- ================================================================
-- WorkOS CRM — Migration 002: Full Functional Schema
...
CREATE TABLE IF NOT EXISTS activity_log (
  id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  entity_type text NOT NULL,   -- 'project','task','file','event','objective'
  entity_id   uuid,
  entity_name text,
  action      text NOT NULL,   -- 'created','updated','deleted','completed','uploaded'
...
-- Add project_id to existing tables (safe — adds column if not exists)
ALTER TABLE folders     ADD COLUMN IF NOT EXISTS project_id uuid REFERENCES projects(id) ON DELETE CASCADE;
ALTER TABLE files       ADD COLUMN IF NOT EXISTS project_id uuid REFERENCES projects(id) ON DELETE CASCADE;
ALTER TABLE gantt_tasks ADD COLUMN IF NOT EXISTS project_id uuid REFERENCES projects(id) ON DELETE CASCADE;
ALTER TABLE gantt_tasks ADD COLUMN IF NOT EXISTS assigned_to text;
ALTER TABLE gantt_tasks ADD COLUMN IF NOT EXISTS priority text DEFAULT 'medium';
...
ALTER TABLE canvases    ADD COLUMN IF NOT EXISTS project_id uuid REFERENCES projects(id) ON DELETE CASCADE;
...
-- ── Helper function: auto-update updated_at on projects ──
...
-- ================================================================
-- Done! All tables created with RLS, indexes, and auto-triggers.
-- ================================================================
